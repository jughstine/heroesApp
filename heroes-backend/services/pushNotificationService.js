...
async function sendPushNotificationToUser(userId, title, body, data = {}) {
  try {
    ...
    if (platform === 'android' && fcmToken) {
      notificationResult = await sendFCMNotification(fcmToken, title, body, data);
    } 
    // Use Expo for iOS or as fallback
    else if (expoToken) {
      notificationResult = await sendExpoNotification(expoToken, title, body, data);
    } 
    else {
...
const sendStatusChangeNotification = async (db, userId, oldStatus, newStatus) => {
  try {
    ...
    if (pensionerResult.length > 0) {
      ...
      if (source_table === 'test_table') {
        nameQuery = 'SELECT FIRSTNAME AS firstname, LASTNAME AS lastname FROM test_table WHERE NDX = ?';
      } else if (source_table === 'test_res_table') {
        nameQuery = 'SELECT FIRSTNAME AS firstname, LASTNAME AS lastname FROM test_res_table WHERE NDX = ?';
      }
    ...
    if (pushResult.success) {
      ...
      if (pushResult.shouldRemoveToken) {
        const tokenToRemove = platform === 'android' && fcm_token ? 'fcm_token' : 'push_token';
        await executeQuery(db, `UPDATE users_tbl SET ${tokenToRemove} = NULL WHERE id = ?`, [userId]);
      }
    }
...
async function sendAnnouncementNotification(tokens, announcement) {
  ...
  if (expoTokens.length > 0) {
    const expoResult = await sendExpoAnnouncementBatch(expoTokens, announcement);
...
async function sendFCMAnnouncementBatch(tokens, announcement) {
  ...
  const notificationBody = announcement.title || 'A new announcement has been posted.';
