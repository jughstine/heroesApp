...
router.post("/login", async (req, res) => {
  try {
    ...
    const createdAtPH = admin.created_at
      ? convertToPHTime(new Date(admin.created_at))
      : null;
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (parsedNavPermissions && parsedNavPermissions.length > 0) {
        const navValues = parsedNavPermissions
          .map((navId) => `(${newAdminId}, ${navId})`)
          .join(",");
      ...
      if (parsedFormPermissions && parsedFormPermissions.length > 0) {
        const formValues = parsedFormPermissions
          .map(
            (fp) =>
    ...
    try {
      ...
      if (req.file) {
        ...
        if (existingAdmin.length > 0 && existingAdmin[0].profile_picture) {
          try {
            const oldKey = existingAdmin[0].profile_picture.split(
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await connection.execute(
        "DELETE FROM admin_nav_access WHERE admin_id = ?",
        [adminId],
      );
      await connection.execute(
        "DELETE FROM admin_form_access WHERE admin_id = ?",
        [adminId],
      );

      if (navPermissions && navPermissions.length > 0) {
        const navValues = navPermissions
          .map((navId) => `(${adminId}, ${navId})`)
          .join(",");
        await connection.execute(
          `INSERT INTO admin_nav_access (admin_id, nav_permission_id) VALUES ${navValues}`,
        );
      ...
      if (formPermissions && formPermissions.length > 0) {
        const formValues = formPermissions
          .map(
            (fp) =>
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Delete the admin
      await connection.execute("DELETE FROM admins_tbl WHERE id = ?", [
        adminId,
      ]);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const [totalResult, activeResult, superResult, regularResult] =
  ...
  try {
    ...
    res.json({
      ...
      meta: {
        ...
        can_assign_to_all:
          currentUserRole === "S_ADMIN" || currentUserRole === "CARES",
      },
    });
  } catch (error) {
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // If restoring from DECEASED, clear deceased flag only if date_deceased is null
      if (oldStatus === "DECEASED" && status !== "DECEASED") {
        await conn.execute(
          `
          UPDATE heroes_tbl h
          INNER JOIN pensioners_tbl p ON p.hero_ndx = h.NDX
          ...
          SET
            h.is_deceased = 0,
            h.date_deceased = NULL
          WHERE u.id = ?
            AND h.date_deceased IS NULL 
        `,
          [userId],
        );
      }
